...
const originalProfile = process.env.SC_CHROME_PROFILE_DIR;

afterEach(() => {
  if (originalProfile === undefined) {
    delete process.env.SC_CHROME_PROFILE_DIR;
  } else {
    process.env.SC_CHROME_PROFILE_DIR = originalProfile;
  }
});
...
describe("resolveChromeProfileDir", () => {
  ...
  test("falls back to shared super-creator profile path", () => {
    delete process.env.SC_CHROME_PROFILE_DIR;
    const resolved = resolveChromeProfileDir();
    if (process.platform === "darwin") {
      expect(resolved).toBe(path.join(os.homedir(), "Library", "Application Support", "super-creator", "chrome-profile"));
    } else if (process.platform === "win32") {
      expect(resolved.endsWith(path.join("super-creator", "chrome-profile"))).toBe(true);
    } else {
      expect(resolved.endsWith(path.join("super-creator", "chrome-profile"))).toBe(true);
    }
  });
