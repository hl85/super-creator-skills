...
```bash
# [示例，暂不可用] ./sc-run sc-multi-publish main article.md --to wechat,x,weibo
# [示例，暂不可用] ./sc-run sc-multi-publish main article.md --to wechat,x,weibo --publish
# [示例，暂不可用] ./sc-run sc-multi-publish main article.md --to x,weibo --skip-review
```
...
| `--skip-review` | off | Skip `sc-content-review` (only if explicitly requested) |

## Pipeline

1.  **Content Review**: Run `sc-content-review` for target platforms.
