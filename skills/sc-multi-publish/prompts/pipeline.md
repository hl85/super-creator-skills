...
```bash
./sc-run sc-content-review main <article.md> --platform <wechat|x> -o <out-dir>/review-<platform>.md
```
...
```bash
./sc-run sc-compress-image main <image-path> --max-size <limit>
```
...
|----------|----------------------------------|------------------------------|
| wechat | invoke `sc-post-to-wechat` with the publisher's built-in draft mode (don't click 发布) | invoke normally |
| x | write `thread.json` only, do NOT spawn the X publisher | invoke `sc-post-to-x` with the JSON |
| weibo | save `post.md` to local drafts; do NOT spawn `sc-post-to-weibo` | invoke normally |
...
```
sc-multi-publish complete (<draft|publish> mode)
out-dir: <path>
...
- ❌ Skipping sc-content-review without `--skip-review`
