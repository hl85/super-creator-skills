...
## Step 1 (sc-content-review) failures

| Symptom | Action |
|---------|--------|
| `sc-content-review` script crashed (exit ≠ 0) | Retry once. If still crashing, abort and tell the user — review can't be skipped silently. |
...
| All images fail (e.g., sc-compress-image script broken) | Log; continue with originals. Don't abort — many articles have no images. |
...
```
sc-multi-publish complete (publish mode)
