...
本文档定义 writeflow 的小红书草稿输出与 xhs-images skill 之间的标准交接规范。
...
| Outline 文件 | 预先生成的 outline（推荐） | writeflow 交接 |
| 命令行参数 | `--style` / `--layout` / `--preset` | 快速生成 |

### 2.2 推荐交接方式：Outline 文件

writeflow 输出的 image spec 需要转换为 xhs-images 的 outline 格式后再调用。

xhs-images outline 格式（见 `xhs-images/references/workflows/outline-template.md`）：
...
Writeflow Draft 输出
  ...
  调用 xhs-images skill
       │
       └─► ./sc-run xhs-images main outline.md --yes
```
...
从 writeflow 输出的 `## 信息图分页内容` 中提取每张图的 YAML spec，校验：
...
将 writeflow 格式转换为 xhs-images outline 格式：
...
按 `xhs-images/references/workflows/outline-template.md` 格式写入 `outline.md`。

**Step 4: 调用 xhs-images**
...
xhs-images 生成的图片默认输出到当前目录的 `outputs/` 或工作目录中，具体路径见 xhs-images 文档。
