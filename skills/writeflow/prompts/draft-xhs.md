...
For each image from the outline, produce a detailed content spec for xhs-images:
...
image:
  ...
  layout: sparse         # from xhs-images layout gallery
  style: bold            # from xhs-images style gallery
...
生成完内容后，将 image content spec 以结构化 YAML 输出，方便后续交给 xhs-images skill 执行。
...
2. **转换为 xhs-images outline 格式**（详见 [references/xhs-to-images-handover.md](../references/xhs-to-images-handover.md)）：
   - 将 YAML images 数组转换为 xhs-images 的 outline markdown 格式
   - 生成 frontmatter（style、image_count 等）
   - 自动生成 slug 和 filename

3. **调用 xhs-images skill**：
   ```bash
...
- [ ] 标签 3-5 个
- [ ] 内容来自真实纪要（无虚构）
- [ ] image spec 字段与 xhs-images 命名一致
