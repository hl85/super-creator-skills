...
```bash
mkdir -p "$(pwd)/publish-wechat/$(date +%Y-%m-%d)"
# Save content to: publish-wechat/yyyy-MM-dd/[slug].md
```
...
**API method** (accepts `.md` or `.html` — pass `.md` directly, no pre-conversion needed):

```bash
./sc-run sc-publish-wechat wechat-api <file> --theme <theme> [--color <color>] [--title <title>] [--summary <summary>] [--author <author>] [--cover <cover_path>] [--no-cite]
```
...
**Browser method** (accepts `--markdown` or `--html` — pass `.md` directly, no pre-conversion needed):

```bash
./sc-run sc-publish-wechat wechat-article --markdown <markdown_file> --theme <theme> [--color <color>] [--no-cite]
./sc-run sc-publish-wechat wechat-article --html <html_file>
```
...
Files created:
[• publish-wechat/yyyy-MM-dd/slug.md (if plain text)]
[• slug.html (converted)]
```
