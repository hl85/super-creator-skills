...
本文档说明 sc-xhs-images 如何接收和处理来自 sc-writer skill 的图片内容规范（image content spec）。

---

## 1. 概述

sc-writer 是内容创作的上游 skill，负责将原始素材 → 平台化的大纲和草稿。当用户使用 sc-writer 完成小红书草稿后，会输出结构化的 image content spec，sc-xhs-images 负责将其转化为实际的图片生成。

### 1.1 输入来源

sc-writer 在 `draft-xhs` 阶段输出的内容结构：

```
sc-writer draft-xhs 输出
├── 标题
├── 正文 (caption)
├── 标签
└── 信息图分页内容 (images 数组)
...
sc-writer 使用 5 种细粒度位置，sc-xhs-images 内部使用 3 种分类：

| sc-writer `position` | sc-xhs-images `Position` | 文件名类型 |
|----------------------|-----------------------|-----------|
| `cover` | `Cover` | `cover` |
...
| sc-writer 字段 | sc-xhs-images 字段 | 转换规则 |
|-----------------|-----------------|----------|
...
当 sc-writer 输出中缺少 `style` 或 `layout` 时（不推荐但可能发生），按以下策略自动补全。
...
**Step 3: 转换为 sc-xhs-images outline 格式**
...
配合 sc-writer 输出的正文和标签，即可发布到小红书。
...
- [writeflow-output-spec.md](writeflow-output-spec.md) - sc-writer 输出格式规范
