...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer `sc-imagine` batch mode (`build-batch.ts` -> `main.ts --batchfile`)
...
Use the available image generation skill (default: `sc-imagine`). If multiple image generation skills are available, ask the user to choose; if none are available, prompt the user to configure an API Key or install an image generation skill.
...
|------------------------|--------|
| Yes (e.g., sc-imagine with Google) | Pass reference images via `--ref` |
