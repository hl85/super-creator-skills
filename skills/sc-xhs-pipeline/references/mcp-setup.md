...
本指南介绍如何安装、配置 xiaohongshu-mcp 服务，以及如何与 sc-xhs-pipeline 集成。
...
## 5. 与 sc-xhs-pipeline 集成配置

### 环境变量配置

sc-xhs-pipeline 通过以下环境变量识别 MCP 服务：
...
sc-xhs-pipeline 的 Stage 4（发布阶段）有两种模式，在 pipeline 启动时选择：
...
| 2️⃣ 次之 | **sc-post-to-xhs CDP** | Chrome 可用且已登录 | Chrome 浏览器自动化脚本 |
...
MCP 不可用时，降级到 sc-post-to-xhs 的 CDP 脚本：

```bash
cd /path/to/sc-post-to-xhs/scripts && npx -y bun xhs-post.ts note \
  --title "笔记标题" \
...
sc-xhs-pipeline 的 Stage 4（发布阶段）会自动检测并使用 MCP：
...
- `cdp` — 使用 sc-post-to-xhs CDP 脚本发布
- `manual` — 手动发布
