...
description: First-time setup and default model selection flow for sc-imagine
---
...
- In `sc-imagine`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `sc-imagine`, `quality` is a compatibility preset. It is not a native DashScope parameter.
