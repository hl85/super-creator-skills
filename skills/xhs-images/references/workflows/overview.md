...
xhs-images skill 的完整工作流程总览，涵盖从输入到输出的全链路。
...
| Writeflow 输出 | 从 writeflow skill 衔接 | 内容创作流水线 | writeflow-input.md |

### 2.1 从 Writeflow 输入

writeflow 是内容创作的上游 skill，负责将原始素材转化为平台化的内容方案。

**输入格式**：writeflow 的 `draft-xhs` 阶段输出的 image content spec（YAML 格式）

**处理步骤**：
1. 解析 writeflow 输出的 images 数组
2. 字段映射与转换（position 5→3 分类等）
3. 生成 xhs-images 标准 outline
4. 执行图片生成
