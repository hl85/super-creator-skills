# Writer → XHS-Images 衔接手册

本文档定义 sc-writer 的小红书草稿输出与 sc-xhs-images skill 之间的标准交接规范。

---

## 1. Writer 输出格式（Image Content Spec）
...
| Outline 文件 | 预先生成的 outline（推荐） | sc-writer 交接 |
| 命令行参数 | `--style` / `--layout` / `--preset` | 快速生成 |

### 2.2 推荐交接方式：Outline 文件

sc-writer 输出的 image spec 需要转换为 sc-xhs-images 的 outline 格式后再调用。
...
从 sc-writer 输出的 `## 信息图分页内容` 中提取每张图的 YAML spec，校验：
...
将 sc-writer 格式转换为 sc-xhs-images outline 格式：
