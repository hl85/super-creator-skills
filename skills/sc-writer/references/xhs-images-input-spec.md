...
> 本文档基于 sc-xhs-images skill 的 `references/workflows/writeflow-input.md` 精简，只保留 sc-writer 需要了解的输入格式约定。完整信息请参考 sc-xhs-images skill 的官方文档。

---

## 1. 概述

sc-xhs-images 是下游 skill，负责将 sc-writer 输出的图片内容规范（image content spec）转化为实际的小红书信息图系列。

sc-writer 在 `draft-xhs` 阶段输出的 image spec，是 sc-xhs-images 的输入来源。

---

## 2. Writer 输出格式（即 XHS-Images 输入）

### 2.1 Image Spec Schema

sc-writer 输出的单张图片规范（YAML 格式）：
...
sc-xhs-images 对接入的 image spec 有以下校验要求，sc-writer 输出时应满足：
...
sc-writer 输出 image spec 后，转换为 sc-xhs-images 的 outline 格式，再调用：
