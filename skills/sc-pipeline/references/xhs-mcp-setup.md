...
本指南介绍如何安装、配置 xiaohongshu-mcp 服务，以及如何与 sc-pipeline 集成。
...
2. 扫码登录
3. 调用 `check_login_status` 确认登录状态
...
4. 调用 `check_login_status` 确认登录状态

### 验证发布功能

```
# 调用 check_login_status 检查登录
check_login_status()
...
## 5. 与 sc-pipeline 集成配置

### 环境变量配置

sc-pipeline 通过以下环境变量识别 MCP 服务：
...
sc-pipeline 的 XHS 平台 Stage 5（发布阶段）有两种模式，在 pipeline 启动时选择：
...
3. 组装发布参数（标题、正文、压缩后 WebP 图片路径、标签）
...
sc-pipeline 的 XHS Stage 5（发布阶段）会自动检测并使用 MCP：

```
Stage 4 (review) 通过 → 进入 Stage 5 (publishing)
                    │
                    ▼
          检测 MCP 服务是否可用
                    │
...
- `cdp` — 使用 sc-publish-xhs CDP 脚本发布
- `manual` — 手动发布
...
- 平台阶段指南：[`platforms/xhs.md`](platforms/xhs.md)
- 状态管理文档：[`./state-management.md`](./state-management.md)
- 硬闸门机制：[`./hard-gates.md`](./hard-gates.md)
