...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer `imagine` batch mode (`build-batch.ts` -> `main.ts --batchfile`)
...
|------------------------|--------|
| Yes (e.g., imagine with Google) | Pass reference images via `--ref` |
