...
Common error recovery strategies for article illustration workflows.
...
1. Use `AskUserQuestion` to ask the user which provider they have and to supply the key.
   Example question: *"文章配图生成需要 API Key，您有哪个服务的 Key？（OpenAI / Google / DashScope / MiniMax 等）"*
...
When image generation is unavailable (no API key, network error, quota exhausted) **and** the illustration request is a logical or structural diagram — flowchart, framework diagram, comparison chart, timeline, mind map, architecture diagram — do not block delivery. Instead:
...
3. Inform the user that the illustration was delivered as a vector diagram instead of a raster image.

This fallback is zero-dependency, produces high-resolution scalable output, and is appropriate whenever photorealistic or painterly aesthetics are not required for the article illustration.
