...
Image generation is delegated to the **available image generation skill (default: `imagine`)**.
...
1. Build batch file: `./sc-run article-illustrator build-batch --input article.md`
2. Generate all images: `./sc-run imagine main --batchfile prompts/batch.json`

## Generation Command

Recommended invocation (via `imagine` skill):

```bash
./sc-run imagine main --promptfile prompts/01-illustration-intro.md
```
