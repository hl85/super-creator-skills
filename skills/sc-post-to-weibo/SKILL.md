...
All commands use `./sc-run sc-post-to-weibo <script>`.

```bash
# Regular post (Text + Image)
./sc-run sc-post-to-weibo weibo-post "Post content" --image ./img.png

# Headline Article (Markdown)
./sc-run sc-post-to-weibo weibo-article article.md --cover ./cover.jpg
```
