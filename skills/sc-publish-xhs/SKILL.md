---
name: sc-publish-xhs
description: Publishes image-text notes (图文笔记) to Xiaohongshu (小红书 / RedNote / XHS) via xiaohongshu-mcp (preferred) or Chrome CDP automation. Accepts a set of images (typically from sc-xhs-images) and a caption, then publishes as a new note. Supports cover image selection, topic tags, and location tagging. Use when user asks to "发小红书", "publish to XHS", "发红书", "publish to RedNote", "xiaohongshu-mcp 发布", "MCP 发小红书", or has sc-xhs-images output ready to publish.
version: 1.2.0
---

# Publish to Xiaohongshu (小红书)

Publishes image-text notes (图文笔记) to Xiaohongshu via xiaohongshu-mcp (preferred) or Chrome CDP browser automation.

## Usage

All commands use `./sc-run sc-publish-xhs <script>`.

**发布方式优先级**：xiaohongshu-mcp → Chrome CDP 脚本 → 手动发布手册

```bash
# MCP 方式发布（优先）
./sc-run sc-publish-xhs mcp-publish --title "标题" --caption "正文" --images img1.png,img2.png

# CDP 方式发布（降级）
./sc-run sc-publish-xhs cdp-publish --title "标题" --caption "正文" --images img1.png,img2.png
```
