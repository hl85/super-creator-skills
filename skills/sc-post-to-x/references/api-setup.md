...
```bash
./sc-run sc-post-to-x x-api "Check out this image!" --image ./photo.png
./sc-run sc-post-to-x x-api "Multiple images!" --image ./1.png --image ./2.png --image ./3.png --image ./4.png
```

### 指定账号

```bash
./sc-run sc-post-to-x x-api "Hello from work account" --account work
```

### 强制使用浏览器模式

```bash
./sc-run sc-post-to-x x-browser "Hello via browser" --image ./photo.png
```
